<?php
/*
 * Template Name: Micro Textile Lab
*/
/** *
 * @package WordPress
 * @subpackage Starkers
	...
	if(isset($background_image) || isset($lead_html)) :
		...
		<?php
			...
			if((int)$theBG==$theBG) $theBGurl=get_ngg_image_url($theBG); 	// is ngg image ID. get URL from NGG functions
				...
				<div id="lead-wrap">		
					<div id="fullscreen-lead" style="background: url(<?php echo $theBGurl; ?>) no-repeat center center scroll;  background-size:cover;">
						<div id="lead-text">
							<h1 class="post-title"><?php the_title(); ?></h1>
							<?php if( $sCustomSubtitle=get_post_meta($post->ID,"subtitle",true) ) echo '<h2 class="post-subtitle">'.$sCustomSubtitle.'</h2>'; ?>
						</div><!-- #lead-text -->
		...
		<div id="lahg2col-container">
			<div id="lahg-center" class="column">
					...
					<div id="post-wrap" >
						...
						<div id="post-body" class="the-content">
							...
							<div class="clear">&nbsp</div>
								...
								<?php echo make_gridster(array('queryValue'=>array($labcategory[0]), 'gridElementBorder'=>1, 'maxColumns'=>1, 'gridElementHeight'=>160, 'flexibleHeight'=>TRUE,'totalWidth'=>640, 'margins'=>array(8,60), 'padding'=>array(8), 'hasInfoTab'=>FALSE, 'imagePosition'=>'left', 'smartImagePlacement'=>FALSE, 'textSpace'=>array(360,120), 'textOptions'=>array('date','title','excerpt'), 'useEmphasis'=>FALSE, 'morelink'=>'http://newyork.thecityatlas.org/category/micro-textile-lab-blog/page/2/', 'numberPosts'=>10 )); ?>
								<?php endif; ?>
						</div><!-- #post-body -->					
						<div id="post-foot">
				...
				<div id="lahg-right" class="column sidebar-border-right">
					...
					<?php if ( function_exists('dynamic_sidebar') && dynamic_sidebar('Micro Textile Lab') ) : ?>
					<?php endif; ?>
