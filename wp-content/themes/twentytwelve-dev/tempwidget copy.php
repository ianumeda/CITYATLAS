...
function gethistoricalaverage($city){
	...
	$query="SELECT * FROM `atlas_temp_widget_newyork` WHERE `date` LIKE '%$todaysmonthday'";
	$result=mysql_query($query) or die(mysql_error());
...
$widgetlink='http://newyork.thecityatlas.org/temperature-widget/';
...
echo "<a href='". $widgetlink ."'><span class='infobutton'>i</span>Today in $city: <span class='concise'><br/></span><span class='high'>". getF($thedata['forecast'][0]) ."&deg;F</span><span class='verbose'><br/>Historical Avg: <span class='high'>". getF($thedata['historicalaverage'][0]) ."&deg;F</span></span><span class='concise'> (".$comparison.") </span></a>";
?>