...
<!-- // Scroll to Anchor //-->
	...
	function getpreview(slug){
		...
		for(i=1; i<window.postdata[slug].length; i++){
			thepreview+='<a href="'+ window.postdata[slug][i].permalink +'"><div class="postpreview"><div class="image" style="display:block; width:100%; height:120px; background: url('+ window.postdata[slug][i].image.url +') no-repeat center center scroll; -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-size: cover;"></div><div class="title previewtext">'+ window.postdata[slug][i].title +'</div><div class="excerpt previewtext">'+ window.postdata[slug][i].excerpt +'</div>';
			if(window.postdata[slug][i].date!="") { thepreview+='<div class="thedate previewtext">'+ window.postdata[slug][i].date +'</div>'; }
	...
	function get_menu_item($post_slug, $title, $cat="", $customargs=null, $item_class="", $title_style="", $customlink=null, $blurb=""){
		...
		if(!is_mobile() && $posts=get_posts($args)){
			...
			if($cat!="no popup"){
				...
				$htmlMenuItem.='<div class="pagepreview" style="position: absolute; right: 0; top: 0; width: 240px; height: 200px; background: white; padding: 10px; box-shadow: -1px 0 8px #000;"><a href="'.$link.'" style="color:inherit; text-decoration:none;"><span class="title">more <span style="color:#8fc54b;">'.$title.'</span> &rarr;</span></a>';
				//$htmlMenuItem.='<span class="image">'.get_fit_image(get_image_from_post_bamn($posts[0],230,180), 230, 180, $link).'</span>';
