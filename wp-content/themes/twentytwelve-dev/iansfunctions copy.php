...
function getjsonpostdata($post){
	...
	return '{"postID":'. $post->ID .',"date":"'. $thedate .'","author":"'. get_the_author($post->ID) .'","permalink":"'. get_permalink($post->ID) .'","title":"'. escapehtmlchars($post->post_title) .'","subtitle":"'.escapehtmlchars(get_post_meta($post->ID, 'subtitle', true)).'","excerpt":"'. escapehtmlchars(get_the_excerpt($post->ID)) .'","image":{"url":"'. escapehtmlchars($arrimage[0]) .'","width":"'. $arrimage[1] .'","height":"'. $arrimage[2] .'"}}';	
}
function getjsonsectiondata($sectiondata){
	...
	foreach($sectiondata as $key=>$section){
		...
		if($posts=get_posts($args)){
			$post=$posts[0];
			setup_postdata($post);
			// if(empty($section['sectiontitle'])) $section['sectiontitle']=$posts[0]->post_title;
			// if(empty($section['blurb'])) $section['blurb']=get_post_meta($posts[0]->ID, 'subtitle', true);
			// if(!empty($section['sectionlink'])) $section['sectionlink']=$sectionlink;
			// else $sectionlink=get_permalink($posts[0]->ID);
...
function makepreviewgrid($queryargs, $elements=array("date","image","title","subtitle","excerpt","author")){
	...
	if ( $q->have_posts() ) {
		while ($q->have_posts()) : $q->the_post();
			...
			$thegrid.='<a href="'. get_permalink($post->ID) .'"><div class="postpreview post-'. $post->ID .'">';
			foreach($elements as $element){
				if($element=="date") {
					if(tribe_is_event($post->ID)) {
						...
						$thegrid.='<div class="thedateevent previewtext">'. $thedate .'</div>';
					}
					else {
						$thedate=mysql2date('M j, Y', $post->post_date);
						$thegrid.='<div class="thedate previewtext">'. $thedate .'</div>';
					}
				}
				elseif($element=="image"){
					$arrimage=get_image_from_post_bamn($post, null, null);
					//echo "bamn:"; print_r($arrimage);
					$thegrid.='<div class="image" style="display:block; width:100%; height:'. $imageheight .'; background: url('. $arrimage[0] .') no-repeat center center scroll; -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-size: cover;"></div>';
				}
				elseif($element=="title") $thegrid.='<div class="title previewtext">'. $post->post_title .'</div>';
				elseif($element=="subtitle") { 
					if($subtitle=get_post_meta($post->ID, 'subtitle', true)) {
						$thegrid.='<div class="subtitle previewtext">'. $subtitle .'</div>'; 
					}
				}
				elseif($element=="excerpt") $thegrid.='<div class="excerpt previewtext">'. get_the_excerpt($post->ID) .'</div>';
				elseif($element=="author") $thegrid.='<div class="author previewtext">by '. get_the_author($post->ID) .'</div>';
			}
			$thegrid.='</div></a>';
