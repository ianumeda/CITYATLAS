...
// version 14: - added sharing button option 'shareButton'
// version 15: (1) removed text-wrapper div and placed art and text inside content_wrapper only so text wraps around image (2) added 'flexibleWidth' , 'gridElementMax/MinWidth' to attempt to have variable widths. Doesn't really work well.
// version 16: (1) added defaultImage
...
function get_gridster_parameters( $aGP=null ) {
	...
	if(!isset( $aGP['margins'] ) ) $aGP['margins'] = 5;
	$aGP['margins']=check_four_element_array($aGP['margins']); 
	
	if(!isset( $aGP['padding'] ) ) $aGP['padding'] = 5;
	...
	if(!isset( $aGP['flexibleWidth'] )) $aGP['flexibleWidth'] = FALSE; // this gives the grid elements percentage widths and max/min limits
	if(!isset( $aGP['gridElementMinWidth'] )) $aGP['gridElementMinWidth']=180;
	if(!isset( $aGP['gridElementMaxWidth'] )) $aGP['gridElementMaxWidth']=960;
	...
	if(!isset($aGP['shareButton'])) $aGP['shareButton']=FALSE; // sharebutton default is off for now for legacy continuity
	if(!isset($aGP['defaultImage'])) $aGP['defaultImage']=''; 
	else if(!is_array($aGP['defaultImage'])) $aGP['defaultImage']=array($aGP['defaultImage'], null, null);// defaultImage places the defined image for posts that have none. strings are converted to array in the format array('url', width, height)
	return $aGP;
}
...
function make_gridster($aGP){
	...
	$htmlGridster='<div class="gridster" style="position:relative;';
	if($aGP['totalHeight']>0 && $aGP['flexibleHeight']) {
		$htmlGridster.=' height:'.$aGP['totalHeight'].'px; overflow:hidden; border-bottom:1px solid;';
	} 
	$htmlGridster.='" >';
...
$gridster[$k]['dimensions']=get_grid_element_dimensions($gridster[$k]['dimensions'],$maxElementDimensions,$aGP['minElementDimensions'],$aGP['dimensionPriority']);
		...
		if($whatToDo[0]=='sort') {
			...
			&& $gridster[$m-1]['float']==$floatOrder[0]) {
				...
				for($q=$m; $q<$k; $q++) { ${'gridmap'.$floatOrder[0]}=erase_element_from_gridmapLR(${'gridmap'.$floatOrder[0]}, $gridster[$q], $aGP['gridDimensions']);
...
function make_gridster_element ($post, $grid_element_dimensions=array('columns'=>1,'rows'=>1), $aGP=null, $grid_element_float="") {
	if(empty($aGP)) $aGP==get_gridster_parameters();
	...
	if($aGP['imageOptions']!=FALSE) {
		...
		if(empty($arrImage)) { 
			if(!empty($aGP['defaultImage'])) $arrImage=$aGP['defaultImage'];
			else $aGP['imageOptions']=FALSE; 
		}
	...
	$htmlGridElement='<div class="grid_element'.$sCategorylist.'" style="border-width:'.$aGP['gridElementBorder'][0].'px '.$aGP['gridElementBorder'][1].'px '.$aGP['gridElementBorder'][2].'px '.$aGP['gridElementBorder'][3].'px; height:'.$nThisGridElementHeight.'px; margin:'.$aGP['margins'][0].'px '.$aGP['margins'][1].'px '.$aGP['margins'][2].'px '.$aGP['margins'][3].'px; padding:0; float: '.$sFloat.'; overflow:hidden; position:relative;';
	if($aGP['flexibleWidth']) $htmlGridElement.=' width:'.(floor(100*1/$aGP['maxColumns'])).'%; max-width:'.$aGP['gridElementMaxWidth'].'px; min-width:'.$aGP['gridElementMinWidth'].'px; min-height:'.($nImageDivHeight+$aGP['padding'][0]+$aGP['padding'][2]+$aGP['imageBorder'][0]+$aGP['imageBorder'][2]).'px;';
	else $htmlGridElement.=' width:'.$nThisGridElementWidth.'px;';
	...
	$htmlGridElement.='<div class="grid_element_content_wrapper image_on_'.$aGP['imagePosition'].'" style="margin:'.$aGP['padding'][0].'px '.$aGP['padding'][1].'px '.$aGP['padding'][2].'px '.$aGP['padding'][3].'px; padding:0;" '.$linkWholeElement.' >'; 

	$sFloatStyle="left";
	if($aGP['imagePosition']=='top') $imagemargin=' margin-bottom:'.$aGP['padding'][0].'px';
	elseif($aGP['imagePosition']=='right') $imagemargin=' margin-left:'.$aGP['padding'][0].'px';
	elseif($aGP['imagePosition']=='bottom') $imagemargin=' margin-top:'.$aGP['padding'][0].'px';
	elseif($aGP['imagePosition']=='left') $imagemargin=' margin-right:'.$aGP['padding'][0].'px';
	$imageStyle='float:'. $sFloatStyle .'; width:'. $nImageDivWidth .'px; height:'.$nImageDivHeight.'px; border-width:'.$aGP['imageBorder'][0].'px '.$aGP['imageBorder'][1].'px '.$aGP['imageBorder'][2].'px '.$aGP['imageBorder'][3].'px; overflow:hidden;'.$imagemargin;
	...
	if($aGP['imagePosition']=="bottom") {
		$htmlGridElement.=$sText;
	...
	if($aGP['hasBottomInfo']) {
		...
		$htmlGridElement.='<div class="grid_bottom_info" style="width:'.($nThisGridElementWidth).'px; height:'.$aGP['bottomInfoHeight'].'px; position:absolute; bottom:0px;">';
		$htmlGridElement.='<span class="byline">By</span><a href="'.esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ).'"><span class="author">'.get_the_author($post->ID).'</span></a>';
//		$htmlGridElement.='<span class="dateline">'. get_the_date('n M Y') .'</span>';
		...
		if($aGP['shareButton']) $htmlGridElement.='<span class="shareButton">'.getGridsterShareButton($post).'</span>';
...
function get_gridster_element_text ($post,$aTextOptions=array('title'=>array('style'=>'h1','link'=>'link to post','maxlength'=>0),'date'=>array('style'=>'h3','link'=>'link to post','maxlength'=>0),'excerpt'=>array('style'=>'p','link'=>'link to post','maxlength'=>0)), $customLink=null, $linkTarget="_self") {
	...
	foreach($aTextOptions as $key=>$value) {
		...
		elseif($key=='tec_date') {
			$tec_date=tribe_get_event_date_format( tribe_get_start_date( $post->ID ), tribe_get_end_date($post->ID) );
		}		
	}
...
// ----------- END gridster stuff ------------||
//------- Gigya share button for gridster ----------
function getGridsterShareButton($post){
	if(function_exists('gigya_signup_page')) {
	    ...
	    $link=getshortlink(get_permalink($post->ID));
		$containerID='componentDiv'.$post->ID;
