...
function get_image_from_post_bamn($post, $nImageDivWidth, $nImageDivHeight)
{
	...
	$attachmentargs = array( 'post_type' => 'attachment', 'numberposts' => 1, 'post_status' => null, 'post_parent' => $post->ID, 'order'=> 'ASC');
	if($attachments=get_posts($attachmentargs))
	{
    $imgsize = (!is_numeric($nImageDivWidth) ? 'thumbnail' : array($nImageDivWidth, $nImageDivHeight) );
    // echo '<!-- get_image_from_post_bamn( '.$post->ID.' , '.$imgsize.' ) -->';
		foreach($attachments as $attachment) 
		{
			$arrImage=wp_get_attachment_image_src( $attachment->ID, $imgsize);
			break;
		}
	} 
	else
...
function make_image($arrImage=null,$nW="",$nH="",$sLink="",$aCaptions=array(FALSE,'caption1','caption2'),$linkTarget="_self")
{
	...
	if(isset($arrImage))
	{
		...
		if(isset($arrImage[3]))
		{
			...
			if($nW!="")	$shortcode='[singlepic id='.$arrImage[3].' w='.$nW.' link='.$sLink.']';
			else $shortcode='[singlepic id='.$arrImage[3].' w= h='.$nH.' link='.$sLink.']';
...
//		if(substr(strtolower($sImage),-3)=="jpg" || substr(strtolower($sImage),-3)=="png" || substr(strtolower($sImage),-3)=="gif") 
		else
		{
			...
			$image_img_tag.='<img src="'.$arrImage[0].'" width="'.$nW.'" height="'.$nH.'" >';
		...
		if($aCaptions[0]) 
		{
			...
			{
				$aDefaults=get_ngg_image_caption($arrImage[3]);
...
function get_fit_image($arrImage, $fitW, $fitH, $sPermalink=null, $linkTarget="_self", $style="")
{
	...
	$htmlFitImage.=make_image($arrImage,$nImageDisplayWidth,$nImageDisplayHeight,$sPermalink,null,$linkTarget);
...
function get_ngg_image_dimensions($imageID){
	...
	if($foo){
		$aDimensions=array();
		$aDimensions[]=$foo['width'];
		$aDimensions[]=$foo['height'];
...
{
    // require_once('simple_html_dom.php'); // nextgen gallery now calls this same file so we don't need to include it here
...
function escapehtmlchars($html){
	return preg_replace( '/\s+/', ' ', str_replace(array('&','<','>','"','\''), array('\&','\<','\>','\"','\\\''), $html) );
}
function fitstring($string,$limit=0,$ender="..."){
	if ($limit>0 && strlen($string) > $limit) { return substr($string, 0, $limit) . $ender; } 
	else { return $string; }
}
function getjsonpostdata($post){
	// expects global post defined
	$arrimage=get_image_from_post_bamn($post, null, null);
	if(tribe_is_event($post->ID)) $thedate=escapehtmlchars(tribe_get_event_date_format(tribe_get_start_date($post->ID, true, 'M j, Y'),tribe_get_end_date($post->ID, true, 'M j, Y')));
	else $thedate=null; //mysql2date('M j, Y', $post->post_date);
	return '{"postID":'. $post->ID .',"date":"'. $thedate .'","author":"'. get_the_author($post->ID) .'","permalink":"'. get_permalink($post->ID) .'","title":"'. fitstring(escapehtmlchars($post->post_title), 50) .'","subtitle":"'.escapehtmlchars(get_post_meta($post->ID, 'subtitle', true)).'","excerpt":"'. escapehtmlchars(get_the_excerpt($post->ID)) .'","image":{"url":"'. escapehtmlchars($arrimage[0]) .'","width":"'. $arrimage[1] .'","height":"'. $arrimage[2] .'"}}';	
}
function getjsonsectiondata($sectiondata){
	$jsonpostdata='window.postdata={};';
	foreach($sectiondata as $key=>$section){
		global $post;
		$args=array( 'name' => $section['slug'], 'post_type' => 'page', 'post_status' => 'publish', 'numberposts' => 1 ); // get page data
		if($posts=get_posts($args)){
			$post=$posts[0];
			setup_postdata($post);
			$jsonpostdata.='window.postdata["'. $section['slug'] .'"]=[';
			$jsonpostdata.=getjsonpostdata($post).',';
			wp_reset_query();

			if(!isset($section['query'])) $sectionquery=array('category_name' => $section['slug'], 'posts_per_page' => 5);
			else $sectionquery=$section['query'];

			$q=new WP_Query();
			$q->query($sectionquery);
			if ( $q->have_posts() ) {
...
function makepreviewgrid($queryargs, $postpreviewformat=array('imageheight'=>200, 'elements'=>array("date","image","title","subtitle","excerpt","author"))){
	$thegrid='<div class="grid">';
	if(!isset($queryargs)) $queryargs='';
	global $post;
	$q=new WP_Query();
	$q->query($queryargs);
	if ( $q->have_posts() ) {
		while ($q->have_posts()) : $q->the_post();
			...
			$thegrid.=getpostpreview($post, $postpreviewformat);
...
function getpostpreview($post, $postpreviewformat=array('imageheight'=>200, 'elements'=>array("date","image","title","subtitle","excerpt","author"))){
	$thepostpreview='<a href="'. get_permalink($post->ID) .'"><div class="postpreview post-'. $post->ID .'">';
	foreach($postpreviewformat['elements'] as $format){
		if($format=="date") {
			if(tribe_is_event($post->ID)) {
				...
				$thepostpreview.='<div class="thedateevent previewtext">'. $thedate .'</div>';
			}
			else {
				$thedate=mysql2date('M j, Y', $post->post_date);
				$thepostpreview.='<div class="thedate previewtext">'. $thedate .'</div>';
			}
		}
		elseif($format=="image"){
			$arrimage=get_image_from_post_bamn($post, null, null);
			$thepostpreview.='<div class="image" style="display:block; width:100%; height:'. $postpreviewformat['imageheight'] .'px; background: url('. $arrimage[0] .') no-repeat center center scroll; -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-size: cover;"></div>';
		}
		elseif($format=="title") $thepostpreview.='<div class="title previewtext">'. $post->post_title .'</div>';
		elseif($format=="subtitle") { 
			if($subtitle=get_post_meta($post->ID, 'subtitle', true)) {
				$thepostpreview.='<div class="subtitle previewtext">'. $subtitle .'</div>'; 
			}
		}
		elseif($format=="excerpt") $thepostpreview.='<div class="excerpt previewtext">'. get_the_excerpt($post->ID) .'</div>';
		elseif($format=="author") $thepostpreview.='<div class="author previewtext">by '. get_the_author($post->ID) .'</div>';
	}
	$thepostpreview.='</div></a>';
	return $thepostpreview;
}
?>