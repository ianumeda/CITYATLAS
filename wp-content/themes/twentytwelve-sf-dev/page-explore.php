...
get_header(); ?>
	...
	<?php while ( have_posts() ) : the_post(); ?>
		...
		<div id="primary" class="showcase">
			...
			<?php 
				...
				else $mapcenter='37.71315858834301,-122.17758178710939'; // default for SF
				if(!is_numeric($zoom)){
	...
	<?php endwhile; ?>
		...
		var cityboundsNE=new L.LatLng(38.72301905397471,-121.14761352539061);
		var cityboundsSW=new L.LatLng(36.72567681977065,-123.72528076171875);
		...
		function onLocationFound(e) {
			...
			// else alert("You aren\'t close enough to put you on this map.")
		}
		...
		<?php
		$colors=array("green","blue","greeninverse","blueinverse","grey");
...
//		var markergreenA = L.divIcon({className: 'markergreenA'});
		...
		var todaydata = <?php echo json_encode($todaydata); ?>;
		    ...
		    for (var i = 0; i < xdata.length; i++) {
				...
				if(todaydata[i]==1) { var marker = L.marker([n[0],n[1]], {icon: this['markerblue'+markerid]}).addTo(map); }
				else if(todaydata[i]==2) { var marker = L.marker([n[0],n[1]], {icon: this['markerblueinverse'+markerid]}).addTo(map); }
				else if(todaydata[i]==-1) { var marker = L.marker([n[0],n[1]], {icon: this['markergrey'+markerid]}).addTo(map); }
				else { var marker = L.marker([n[0],n[1]], {icon: this['markergreen'+markerid]}).addTo(map); }
