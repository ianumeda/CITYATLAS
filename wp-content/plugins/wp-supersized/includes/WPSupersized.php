    ...
    public static function slides_list()
{
	if(is_front_page()||is_home()){
		// this takes from the cityatlas 'front-page-feature' category of posts  
		$imagecounter=0;
		$idObj = get_category_by_slug('front-page-features'); 
		$catID = $idObj->term_id;
		...
		{
			setup_postdata($post);
			if($featuredimagecustomfield=get_post_meta($post->ID, 'front_page_feature_url', TRUE)) 
			{
				// handles a direct URL to the image via custom field
				$featuredimageurl=$featuredimagecustomfield;
			}
			elseif (has_post_thumbnail( $post->ID ) )
			{
				// 'featured image' in post is secondary option for image
				$featuredimage=get_post_thumbnail_id( $post->ID );
				if(substr($featuredimage,0,4)=='ngg-')
				{
					$featuredimagenggid=substr($featuredimage,4);
					$featuredimageurl=get_ngg_image_url($featuredimagenggid);
					$arrnggcaption=get_ngg_image_caption($featuredimagenggid);
					$nggcaption="";
					foreach($arrnggcaption as $caption)
					{
						if(!empty($caption)) $nggcaption.=$caption;
					}
				}
				elseif(is_numeric($featuredimage))
				{
					$featuredimagearr = wp_get_attachment_image_src( $featuredimage, 'full' ); 
			...
			{
				$order=(int)get_post_meta($post->ID,'front_page_feature_order',TRUE);
				if(empty($order)) $order=-1;
				else $order=$order-1; // sets to zero-index
				$time=strtotime($post->post_date);
				$link=get_permalink($post->ID);
				$titletext="";
				$caption=get_post_meta($post->ID,'front_page_feature_caption',TRUE);
				// default caption will be set by custom field 'front_page_feature_caption' otherwise depend on ngg or media library
				if(empty($caption))
				{
					if(isset($nggcaption)) $caption=$nggcaption;
					elseif(isset($imgcaption)) $caption=$imgcaption;
				}
				$titletext.=$caption;
				if(empty($titletext)) $titletext=$post->post_title; //.' by '.$post->user_nicename;
				$nolink=get_post_meta($post->ID, "donotlink-frontpagefeature",TRUE); 
				$thelink=get_post_meta($post->ID, "front_page_feature_link", TRUE); // this is the new custom field which enables you to link to any URL you want or no link if you just enter anything other than 'http://' at the beginning of the value
				if(empty($thelink) && empty($nolink)) $linktext='<a href="'.$link.'">read more &rarr;</a>';
				elseif(!empty($thelink) && substr($thelink,0,7)=='http://') $linktext='<a href="'.$thelink.'">read more &rarr;</a>';
				else $linktext="";

				if($order>=0) 
				{
					while(!empty($arrordered[$order])) { $order++; }
					$arrordered[$order]="\n{image : '".$featuredimageurl."', title : '".$titletext." ".$linktext."', url : '".$link."'}";
				}
				else $arrunordered[]="\n{image : '".$featuredimageurl."', title : '".$titletext." ".$linktext."', url : '".$link."'}";

				// if($imagecounter>0) echo ",";
				// echo "\n{image : '".$featuredimageurl."', title : '".$titletext." ".$linktext."', url : '".$link."'}";
				// $imagecounter++;
			}
		}
		// now merge the unordered list into the ordered list...
		// but first shuffle the unordered array...
		shuffle($arrunordered);
		for($i=0; $i<count($arrunordered); $i++)
		{
			$j=0;
			while(!empty($arrordered[$j])) { $j++; }
			$arrordered[$j]=$arrunordered[$i];		
		}
		// now output the ordered list in the correct format...
		for($i=0; $i<count($arrordered); $i++)
		{
			if(!empty($arrordered)) echo $arrordered[$i];
			if($i<count($arrordered)-1) echo ",";
		}
	}
	else
	{
